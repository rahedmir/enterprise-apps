...
{
    public partial class frm_domain_support_asset_desktop : Form
    {
        public frm_domain_support_asset_desktop()
        {
            InitializeComponent();
        }
        ...
        private void frm_domain_support_asset_desktop_Load(object sender, EventArgs e)
        {
            txtYear.Text = "";
            txtNum.Text = "";
        }
        ...
        private void btnRename_Click(object sender, EventArgs e)
        {
            ...
            {
                string num = txtYear.Text + txtNum.Text;

                if (txtYear.Text == "" || txtNum.Text == "")
                {
                ...
                else
                {
                    ...
                    bool isMatch = Regex.IsMatch(num, pattern);

                    if (isMatch)
                    {
                        if (num.Length == 8)
                        {
                            SetHostName($"IBI{txtYear.Text}BHK{txtNum.Text}");
                        }
                        else
                        {
                    ...
                    {
                        MessageBox.Show("Please enter only numbers.", "Domain Support", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
